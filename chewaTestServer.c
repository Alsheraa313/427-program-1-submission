...
// SQLite callback for printing results to the console.
// This function is used when running general SQL messages for testing or debugging.
...
void handleBuyCommand(sqlite3* db, int clientSocket, char* args, const char* serverPrompt) {
    ...
    // If a seller is found, store their ID and how many cards they have
    if (sqlite3_step(stmt) == SQLITE_ROW) {
...
int main() {
    ...
    // Insert default pokemon cards if they don't already exist
    ...
    while (1) {
        ...
        // Print client message on the server side
        printf("RECEIVED: %s\n", clientMessage);
